...
3. Identifique a versão de Node exigida pelo projeto em `.nvmrc`, `package.json` ou configuração equivalente. Quando a versão atual não atender ao requisito, use o NVM para instalá-la, se necessário, e selecioná-la antes de instalar dependências, testar ou executar scripts; confirme a versão com `node --version`.
...
7. Valide apenas no escopo necessário durante a implementação e finalize com `yarn lint`, `yarn test` e, para mudanças HTTP, dados ou integração, `yarn test:e2e`. Informe limitações de ambiente (Oracle, Redis, credenciais ou serviços externos) sem ocultá-las.
