...
7. Valide apenas no escopo necessário durante a implementação e finalize com `yarn lint`, `yarn test` e, para mudanças HTTP, dados ou integração, `yarn test:e2e`. Antes de qualquer commit, execute o comando de lint do projeto com correção automática (`yarn lint --fix` ou o equivalente definido pelo repositório) e revise as alterações produzidas antes do stage. Execute os comandos na versão de Node confirmada no passo 3; se algum não puder ser executado, informe o motivo e a falha observada, sem apresentá-lo como validado. Informe limitações de ambiente (Oracle, Redis, credenciais ou serviços externos) sem ocultá-las.
...
- Faça os casos de uso dependerem de interfaces de gateway, e deixe TypeORM, Oracle, Redis, Azure e APIs externas na infraestrutura. A infraestrutura limita-se a adaptar integrações e a persistir ou transportar dados; regras de negócio nela são proibidas, inclusive decisões condicionais que definam resultado de negócio. Em gateways, não implemente manipulação de dados de domínio (parsing, normalização, filtro, agregação ou transformação orientada à regra), regras de negócio nem tratamento, criação ou tradução de mensagens de erro. Essas responsabilidades pertencem à `application`; o gateway apenas propaga a falha técnica conforme o contrato.
...
- Não use nem introduza `any` em código de produção, inclusive em genéricos, retornos, casts ou declarações de bibliotecas. O uso fica permitido exclusivamente em testes e mocks. No código de produção, modele o contrato com um tipo ou interface apropriado; quando a forma do dado não puder ser definida com segurança, use `unknown` e faça o estreitamento necessário antes do consumo. Não desabilite regras de TypeScript ou lint para contornar essa exigência.
