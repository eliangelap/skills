...
| `any-param.rule.ts` | Qualquer `any` em parametro, retorno, generic, cast, variavel ou declaracao de biblioteca de codigo de producao deve ser comentado. | Permitido exclusivamente em testes e mocks. Fora desses contextos, exigir tipo/interface que modele o contrato ou, quando isso nao for possivel com seguranca, `unknown` com estreitamento antes do consumo. |
...
- Em gateways, manipulação de dados de domínio (parsing, normalização, filtro, agregação ou transformação orientada à regra), regras de negócio e tratamento, criação ou tradução de mensagens de erro. Exigir que esses comportamentos sejam movidos para `application/`; o gateway deve apenas adaptar a integração e propagar a falha técnica conforme o contrato.
...
| 8 | Sem `any` em codigo de producao; permitir exclusivamente em testes e mocks. Sem `import * as` salvo exigencia da lib; consolidar em uma unica declaracao todos os imports do mesmo arquivo e usar `type` nos especificadores quando aplicavel | Fora de testes e mocks, apontar `as any`, `: any`, `any[]`, `Promise<any>` e demais usos. Exigir tipo/interface adequado ou, quando o contrato nao puder ser definido com seguranca, `unknown` com estreitamento antes do consumo; nao aceitar desabilitacao de TypeScript ou lint como contorno. |
