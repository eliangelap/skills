...
| `any-param.rule.ts` | Qualquer `any` em parametro, retorno, generic, cast ou variavel de codigo de producao deve ser comentado. | Nao sinalizar `any` em testes unitarios, testes E2E ou seus arquivos de apoio. Em codigo de producao, permitir apenas em `main.ts`, `@worker/config.ts`, `common/infra/{mock,http,observability,findSimilarity}.ts`, `convertFileBase64.use.case.ts`, `database.service.ts`, `seeds/` e libs `@uniface/*` |
...
| 8 | Em codigo de producao, sem `any`; sem `import * as` salvo exigencia da lib; consolidar em uma unica declaracao todos os imports do mesmo arquivo e usar `type` nos especificadores quando aplicavel | Apontar `as any`, `: any`, imports amplos injustificados e multiplas declaracoes de import para o mesmo arquivo somente fora de testes unitarios, E2E e seus arquivos de apoio |
