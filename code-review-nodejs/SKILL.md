...
| `long-function.rule.ts` | Funcao ou arrow com mais de 20 linhas deve ser comentada e sugerir extracao. | Permitido em `*.container.registry.ts`, `main.ts`, `@worker/config.ts`, `common/infra/{mock,http,observability}.ts`, `documentation.decorator.ts`, scripts de migration |
