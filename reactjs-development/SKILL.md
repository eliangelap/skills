...
5. Trabalhe em TDD: escreva ou ajuste o spec, confirme a falha, implemente o mínimo necessário e valide em verde. Prefira o subdiretório `__test__` da camada ou do diretório do arquivo testado; a regra do projeto também aceita o spec irmão do arquivo. Use a terminação `.spec.ts` ou `.spec.tsx`, conforme a extensão do código testado. Ao criar ou alterar artefatos executáveis de `@core`, mantenha specs relevantes para cada artefato alterado, incluindo domínio, aplicação, infraestrutura, registry e `key.ts` quando aplicável; não crie specs para mocks que só forneçam dados aos testes.
...
- Faça casos de uso dependerem de interfaces de gateway. `infra` apenas adapta HTTP ou outras integrações e implementa o contrato; regras de negócio nela são proibidas, inclusive decisões condicionais que definam resultado de negócio. Gateways não devem manipular dados de domínio (parsing, normalização, filtro, agregação ou transformação orientada à regra), aplicar regras de negócio, nem tratar, criar ou traduzir mensagens de erro; devem propagar a falha técnica conforme o contrato. Toda regra, parsing, validação ou normalização pertence a `application` e deve estar em um caso de uso, com contrato e testes próprios; quando for responsabilidade distinta, crie um caso de uso separado e componha-o explicitamente no fluxo principal.
- Quando uma integração precisar ser temporariamente substituída por dados mockados compatíveis com o contrato, declare o dado tipado em `domain/entities` e faça o gateway apenas retorná-lo em uma nova instância. Não declare dados mockados dentro de adaptadores de infraestrutura.
...
- Ao criar um caso de uso, derive o nome da classe exatamente do nome do arquivo: remova `.use.case.ts`, converta cada segmento kebab-case para PascalCase e acrescente `UseCase`. Exemplo: `get-status.use.case.ts` deve exportar `GetStatusUseCase`.
...
- Não use `style={{...}}` em `.tsx`; concentre estilos em `styles.ts` ou `*.styles.ts` e referencie-os pelo objeto de estilo.
