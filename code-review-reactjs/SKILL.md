...
8. Sem `any` em código de produção; permitir exclusivamente em testes e mocks. Sem `import * as` sem motivo; usar `import type` para tipos puros. Fora de testes e mocks, apontar `as any`, `: any`, `any[]`, `Promise<any>` e demais usos; exigir tipo/interface adequado ou, quando o contrato não puder ser definido com segurança, `unknown` com estreitamento antes do consumo. Não aceitar desabilitação de TypeScript ou lint como contorno.
