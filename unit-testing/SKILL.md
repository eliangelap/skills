---
name: unit-testing
description: "Unit test generation for Coamo-style backend projects using NestJS + TypeORM + layered @core/@server/@worker architecture. Use when writing or expanding specs for entities, DTOs, use cases, gateways, container registries, controllers, workers, or e2e flows in api-coamo-sports and similar repositories."
...
- Read `references/how_to_do.md` when the task involves creating or extending a module and the tests need to follow the exact Coamo build order.
...
- Use the `modality` module as the canonical style reference when the current repository follows the Coamo pattern.
